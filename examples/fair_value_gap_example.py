...
"""
Fair Value Gap (FVG) Strategy Example
------------------------------------

This example demonstrates how to use the Fair Value Gap (FVG) strategy
with either live data or backtesting.

The FVG strategy identifies areas of price imbalance where minimal trading 
activity has occurred, which often act as zones the price will return to "fill"
...
logger.add("logs/fair_value_gap_{time}.log", rotation="500 MB", level="DEBUG")
...
def test_live_strategy():
    """Test the Fair Value Gap strategy with live data"""
    try:
        # Load configuration
        config = load_config()
        ...
        # Create Fair Value Gap strategy
        strategy_config = config['strategies']['fair_value_gap']
        strategy_config['name'] = 'FVG Strategy Live Test'
        ...
        fvg_strategy = create_strategy('fair_value_gap', strategy_config)
        ...
        try:
            ...
            logger.info(f"Starting live testing with FVG strategy on {symbols}")
            
            while True:
                for symbol in symbols:
                    # Get latest data
                    bars = 200  # Need enough history for FVG detection
                    ...
                    signal = fvg_strategy.analyze(symbol, data)
                    ...
                    if signal['signal'] != 'none':
                        ...
                        # Log FVG details if available
                        if 'fvg' in signal:
                            fvg = signal['fvg']
                            logger.info(f"FVG type: {fvg['type']}, Range: {fvg['lower']:.5f} to {fvg['upper']:.5f}, Size: {fvg['size']:.5f}")
                    else:
...
def backtest_strategy():
    """Run backtesting for the Fair Value Gap strategy"""
    try:
        # Load configuration
        config = load_config()
        ...
        # Create Fair Value Gap strategy
        strategy_config = config['strategies']['fair_value_gap']
        strategy_config['name'] = 'FVG Strategy Backtest'
        ...
        # Create backtester
        backtester = MT5Backtester(connector, fvg_strategy)
        ...
        if 'equity_curve' in results:
            ...
            plt.savefig("fvg_equity_curve.png")
            logger.info("Equity curve saved to fvg_equity_curve.png")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fair Value Gap Strategy Example")
    parser.add_argument("--mode", choices=["live", "backtest"], default="live",
                        help="Test mode: live or backtest")
